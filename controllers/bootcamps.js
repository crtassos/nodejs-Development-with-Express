...
exports.updateBootcamp = asyncHandler(async (req,res,next)=>{
    ...
    // Make sure user exists in db and he is admin
    if(bootcamp.user.id.toString() !== req.user.id && req.user.role!== 'admin'){
        next(new ErrorResponse(`User with id ${req.params.id} is not authorized to udate this Bootcamp`
            ,401))
    }
...
exports.deleteBootcamp = asyncHandler(async (req,res,next)=>{
    ...
    if(!bootcamp){
     ...
     // Make sure user exists in db and he is admin
     if(bootcamp.user.id.toString() !== req.user.id && req.user.role!== 'admin'){
        next(new ErrorResponse(`User with id ${req.params.id} is not authorized to delete this Bootcamp`
            ,401))
    }
...
exports.bootcampPhotoUpload = asyncHandler(async (req,res,next)=>{
    ...
    if(!bootcamp){
     ...
     // Make sure user exists in db and he is admin
     if(bootcamp.user.id.toString() !== req.user.id && req.user.role!== 'admin'){
        next(new ErrorResponse(`User with id ${req.params.id} is not authorized to upload a photo`
            ,401))
    }
   ...
   file.mv(`${process.env.FILE_UPLOAD_PATH}/${file.name}`, async (err)=>{
    if(err){
        console.error(err)
        return next(new ErrorResponse(`Please file upload`, 
        400))
    }
   })
